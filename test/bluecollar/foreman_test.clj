...
(deftest foreman-start-stop-workers-test
  ...
  (testing "on start it creates a RedisConnection with the approprate number of pooled threads"
    (let [a-foreman (foreman/new-foreman testing-queue-name 17)
          _ (startup a-foreman)
          redis-conn @(:redis-conn a-foreman)
          redis-pool (get-in redis-conn [:pool :pool])]
      (is (= 17 (.getMaxActive redis-pool)))
      (is (= 17 (.getMinIdle redis-pool)))
      (shutdown a-foreman)
      )))
